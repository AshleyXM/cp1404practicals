...
def main():
    ...
    unreliable_car_b = UnreliableCar("Car B", 500, 60)
    drive_count_a = 0
    drive_count_b = 0
    for _ in range(ITERATION_TIMES):
        ...
        if distance_driven_a != 0:
            drive_count_a += 1
    for _ in range(ITERATION_TIMES):
        distance_driven_b = unreliable_car_b.drive(4)
    ...
    print(f"Total count: {ITERATION_TIMES}. Car B Drive count: {drive_count_b}")
